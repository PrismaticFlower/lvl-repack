...
public:
   ...
   {
      return static_cast<std::uint32_t>(_out.tellp());
   }
...
inline void write_at_alignment(writer& writer, const std::span<const std::byte> data) noexcept
{
   ...
   writer.write(static_cast<std::uint32_t>(aligned_offset));

   // Pad until aligned.
   writer.pad_unaligned(static_cast<std::uint32_t>(aligned_offset));
...
inline auto open_file_for_output(const std::filesystem::path& file_path) -> std::ofstream
{
   ...
   if (!file.is_open()) {
      throw std::runtime_error{
         std::format("Unable to open file {} for output."sv, file_path.string())};
   }
